...
for i=1:szK
    K=KMeansKVals(i);
    %Clustering Algorithm
    [ClusterIDs,Centroids] = kmeans(InputMatrix, K, 'Weight', InputWeights, 'Distance', 'sqeuclidean');
    %Create the Clusters Structure
    Clusters=struct('Traces',[],'Centroid',[]);
    maxClusterID=max(ClusterIDs);
    for j=1:maxClusterID
    ...
    InitialClusterNum=ClusterNum;
    %Evaluate Per Cell
    [~,F1score(:,n),Recall(:,n),Precision(:,n),TruePositiveRate(:,n),FalsePositiveRate(:,n)]=mnl_EvaluateClassifierPerCell(efPxTrace,ClusterIDs,Clusters,dim,'n',KMeansKVals(i),1);
    Setting(n,1)=K;
    n=n+1;
end
...
for i=1:szEps
    for j=1:szMinPts
        ...
        MinPts=MinPtsVals(j);
        %DBSCAN
        D = pdist2(InputMatrix);
        DB = dbscan(D,Eps,'MinPts','MinPts');
        ...
        InitialClusterNum=ClusterNum;
        %Evaluate Per Cell
        [~,F1score_DBSCAN(:,n),Recall_DBSCAN(:,n),Precision_DBSCAN(:,n),TruePositiveRate_DBSCAN(:,n),FalsePositiveRate_DBSCAN(:,n)]=mnl_EvaluateClassifierPerCell(efPxTrace,ClusterIDs,Clusters,dim,'n',Eps,MinPts);
        Setting(n,:)=[Eps,MinPts];
        n=n+1;
    end
end
