function [FragmentValues,BkgMean,MaxValues]=mnl_CalculateMaxValues_v3(Trace,Scale,Data,SampleBkgVxList,NumLengthThresh,SubFrag)
% Calculate the the Maximum Values of each trace, and also calculates the
% fragment values, including the split sections for measuring the
% brightness quality
%
%Inputs
...
% SampleTraceVxList - The sample list of voxels to represent the trace
% NumLengthThresh - The threshold length
% SubFrag - Calculate the fragments from the traces
%
%Outputs
% FragmentValues - Calculate the mean color of each sub fragment and
% measure the distance to the ground truth.
%
% BkgMean - The median background colour
% MaxValues - The maximum intensity value of all the traces
%
%Marcus Leiwe, Kyushu University - 05/03/2022
...
for i=1:szT %Do this for each trace
    ...
    AllPointsList=Trace(i).Points; %Extract the Points Desired (this is in voxels)
...
%Calculate the background median from the sample
...
for i=1:szT
    FragmentValues(i).All.BkgRemovedMean=FragmentValues(i).All.Raw.Mean-BkgMean;
...
tMatrix=nan(szT,dim(3));
for i=1:szT
    NormMean=FragmentValues(i).All.BkgRemovedMean./MaxValues;
    FragmentValues(i).All.NormMean=NormMean;
    FragmentValues(i).All.NormMeanMagnitude=vectorNorm(NormMean);
    tMatrix(i,:)=FragmentValues(i).All.NormMean;
end
%Then Vector Normalise
for i=1:szT
    FragmentValues(i).All.VecNormMean=mnl_SingleVectorNormalise(tMatrix(i,:));
end
%% Now calculate fragment colours
%% Now measure the spread of colours for traces and fragments
if strcmp(SubFrag,'y')==1
    disp('Fragmenting Traces...')
    NumTraces=size(Trace,2);
    %Calculate the desired split for each trace
    FragSubDiv=struct('Frag',[]);
    for i=1:NumTraces
        nPoints=size(Trace(i).Points,1);
        Points=Trace(i).Points;
        nAccFrag=nan(1,nPoints);
        for j=1:nPoints
            nGroups=floor(nPoints/j);%Total number of potential groups
            ed=0;
            PointDivisions=nan(nGroups,2);
            Distances=nan(1,nGroups);
            for k=1:nGroups
                st=ed+1;
                ed=st+(j-1);
                PointDivisions(k,:)=[st ed];
                tPoints=Points(st:ed,:);
                Distances(k)=mnl_MeasureDistaceOfPoints(tPoints);
            end
            idx=find(Distances>=NumLengthThresh);
            nAccFrag(j)=size(idx,2);
            FragSubDiv(i).Frag(j).NumPoints=j;
            FragSubDiv(i).Frag(j).PointDivisions=PointDivisions;
            FragSubDiv(i).Frag(j).Distances=Distances;
            FragSubDiv(i).Frag(j).NumGoodFrags=nAccFrag(j);
        end
        [~,I]=max(nAccFrag);
        FragSubDiv(i).OptFragSize=I;
    end
    %Now begin measuring the spread
    for i=1:NumTraces
        nPoints=size(Trace(i).Points,1); %Total Number of points
        OptSplit=FragSubDiv(i).OptFragSize;
        for j=OptSplit
            FragmentValues(i).FragSize(j).PointNum=j;
            % Calculate the all the potential groups of points
            nGroups=floor(nPoints/j);%Total number of potential groups
            if nGroups==1
                st=FragSubDiv(i).Frag(j).PointDivisions(1,1);ed=FragSubDiv(i).Frag(j).PointDivisions(1,2);
            ...
            elseif nGroups>1 %If there is a possibility to generate more than one group
                ...
                GroupIDs=FragSubDiv(i).Frag(j).PointDivisions;
